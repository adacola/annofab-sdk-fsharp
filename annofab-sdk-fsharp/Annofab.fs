...
module Annofab =
    ...
    type GetStatisticsTaskPhasesResponse = JsonProvider<"""[{"project_id":"str","date":"date","phases":[{"phase":"str","worktime":"timespan"}]}]""">
    ...
    let getStatisticsTaskPhases (baseUri : Uri) token projectID =
        let apiUri = Uri(baseUri, sprintf "projects/%s/statistics/task-phases" projectID)
        let sendApi token =
            let response = Http.Request(url = apiUri.AbsoluteUri, httpMethod = "GET", headers = getJsonAuthHeaders token, responseEncodingOverride = "UTF-8", silentHttpErrors = true)
            response, token
        let parseResponse = getTextResponse >> GetStatisticsTaskPhasesResponse.Parse
        sendAndRefreshToken baseUri sendApi parseResponse token
